 ...
 * 
 * 
 * Solve Poisson's equation: “- laplacian U = F” with Dirichlet boundary conditions
 * using Finite Element Method (FEM)
 * 
 * -( Uxx + Uyy ) = F(x,y)     in the region D
 *         U(x,y) = G(x,y)     on the region boundary #D
 * 
 * The integral form (notice Integral_D is a double integral):
 * 
 * - Integral_D( laplacian U(x,y) dA ) = Integral_D ( F(x,y) dA )
 * 
 * where laplacian U(x,y) = div ( grad U(x,y) )
 * 
 * WEAK FORM
 * 
 ...
 * 
 * V(x, y) R(x,y) = 0
 * V(x, y) (- laplacian U(x,y) - F(x,y)) = 0
 * - V(x,y) laplacian U(x,y) = F(x,y) V(x,y)
 * 
 * and the integral (variational) form of the PDE is
 * 
 * - Integral_D( V(x,y) laplacian U(x,y) dA ) = Integral_D ( F(x,y) V(x,y) dA )
 * 
 ...
 * 
 * -[ Integral_#D ( V <grad U, n> dS ) - Integral_D( < grad V, grad U> dA) ] = Integral_D ( F V dA )
 * 
 * giving:
 * 
 * Integral_D( < grad V, grad U> dA)  = Integral_D ( F V dA ) + Integral_#D ( V <grad U, n> dS )
 * 
 * 
 * 
 ...
 * 
 * Integral_D( < grad V(x,y), grad U(x,y)> dA)  = Integral_D ( F(x,y) V(x,y) dA )
 * 
 * 
 * 
 * DISCRETIZATION
 ...
 * 
 * Integral_Di( < grad K_i(x,y), grad U(x,y)> dA)  = Integral_Di ( F(x,y) K_i(x,y) dA )
 * 
 ...
 * 
 * sum_j U_j  Integral_Di( <grad H_j, grad K_i> dA) = Integral_D i( F K_i dA)
 * 
 * Which can be written as:
 * 
 * M_ij = Integral_Di( <grad H_j, grad K_i> dA
 * F_i   = Integral_Di ( F K_i dA)
 * 
 * M_ij U_i = F_i
 * 
...
std::shared_ptr<SparseMatrix> A, A2;
 ...
 */
Eigen::Matrix3d AssembleElement(Vertex* v[3]) {
...
/**
 * Alternative method of compuring the per-element stiffness matrix
 * as described in:
 * 
 * Francisco-Javier Sayas, "A gentle introduction to the Finite Element Method", 2015
 */
Eigen::Matrix3d AssembleElementAlt(Vertex* v[3]) {

	Eigen::Matrix2d B, Binv;
	Eigen::Matrix3d elementMatrix, K_uu, K_vv, K_uv;

	K_uu.setZero();
	K_vv.setZero();
	K_uv.setZero();

	K_uu(0, 0) = 0.5; K_uu(0, 1) = -0.5;
	K_uu(1, 0) = -0.5; K_uu(1, 1) = 0.5;

	K_vv(0, 0) = 0.5; K_vv(0, 2) = -0.5;
	K_vv(2, 0) = -0.5; K_vv(2, 2) = 0.5;

	K_uv(0, 0) = 0.5; K_uv(0, 2) = -0.5;
	K_uv(1, 0) = -0.5; K_uv(1, 2) = 0.5;
	...
	Binv = B.inverse() * B.inverse().transpose();

	double detB = B.determinant();

	elementMatrix = abs(detB) * (Binv(0,0) * K_uu + Binv(1,1) * K_vv + Binv(0,1) * (K_uv + K_uv.transpose()));
 ...
 * Francisco-Javier Sayas, "A gentle introduction to the Finite Element Method", 2015
 */
Eigen::Matrix3d AssembleElementEmbedded(Vertex* v[3]) {
 ...
 * Francisco-Javier Sayas, "A gentle introduction to the Finite Element Method", 2015
 */
std::shared_ptr<SparseMatrix> AssembleMatrix(Mesh *mesh) {
	...
	for (Face& face : mesh->getFaces()) {
		...
		elementMatrix = AssembleElementEmbedded(v);
		//elementMatrix = AssembleElement(v);
		for( int i = 0 ; i < 3 ; ++i ) {
			for (int j = 0 ; j < 3 ; ++j ) {
				if (i < j) continue; // since stifness matrix is symmetric
				wij = elementMatrix(i, j);
				(*A)(v[i]->ID, v[j]->ID) += wij;
				if (i != j) {
					(*A)(v[j]->ID, v[i]->ID) += wij;
				}
...
int main(int argc, char* argv[])
{
	...
	//A = CreateLaplacianMatrix(&mesh, LaplaceBeltrami);  // reference cotan-laplace-beltrami operator matrix
